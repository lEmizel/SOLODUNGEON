...
const DETAIL_A_CHANCE: float = 0.65  # 65%
...
func _scan_textures() -> void:
	print("sa bug ou pas je veux voir si ce print fonctionne")
	...
	while file_name != "":
		print("Trouvé: ", file_name)  # DEBUG - affiche TOUT ce qui est dans le dossier
...
func _try_load_texture(file_name: String) -> void:
	...
	print("DEBUG: fichier=", file_name, " | prefix=", prefix)  # DEBUG
	...
	if tex:
		...
		print("  -> Chargé OK")  # DEBUG
	else:
		print("  -> ECHEC chargement")  # DEBUG
...
func _generate_floors() -> void:
	for pos in DUNGEONREFERENCE.rooms.keys():
		...
		# Detail A - 65% de chance
		if randf() < DETAIL_A_CHANCE:
			var tex = _pick_random_texture("detaila")
			if tex:
				var sprite = Sprite2D.new()
				sprite.texture = tex
				sprite.centered = true
